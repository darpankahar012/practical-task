...
export default function CartModal({ open, setOpen, products, total }) {
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                  <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                    ...
                    <div className="mt-2 w-full h-3/6">
                      <PDFViewer height="100%" width="100%" showToolbar={true}>
                        <CartPDF products={products} total={total} />
                      </PDFViewer>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  ...
                  >
                    <PDFDownloadLink
                      document={<CartPDF products={products} total={total} />}
                      fileName="summary.pdf"
                      onClick={() => setOpen(false)}
                    >
                      {({ blob, url, loading, error }) => (loading ? 'Loading...' : 'Download summary')}
                    </PDFDownloadLink>
                  </button>
                </div>
              </Dialog.Panel>
